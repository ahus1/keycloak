...
public class KeycloakServiceDependentResource extends CRUDKubernetesDependentResource<Service, Keycloak> {
    ...
    private ServiceSpec getServiceSpec(Keycloak keycloak) {
        ...
        Optional<ManagementSpec> managementSpec = Optional.ofNullable(keycloak.getSpec()).map(KeycloakSpec::getManagementSpec);
        boolean isManagementEnabled = managementSpec.map(ManagementSpec::isEnabled).orElse(false);
        if (isManagementEnabled) {
            builder.addNewPort()
                    .withPort(managementSpec.map(ManagementSpec::getPort).orElse(Constants.KEYCLOAK_MANAGEMENT_PORT))
